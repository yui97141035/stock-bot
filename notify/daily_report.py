...
每日開盤前 + 收盤後報告
包含：技術面、歷史勝率、月營收、EPS、情緒分析 綜合進場判斷
"""
...
from notify.discord_bot import send_message, STOCK_LIST, ETF_LIST, HOLDINGS
...
def historical_analysis(df: pd.DataFrame, short=5, long=20) -> dict:
    ...
    for hd in hold_days:
        ...
        for idx in entry_dates:
            ...
            if pos + hd < len(df):
                ret = (df['close'].iloc[pos + hd] / df['close'].iloc[pos] - 1) * 100
                returns.append(ret)
                if ret > 0:
                    wins += 1
                total += 1
...
def analyze(sid: str, name: str, is_etf: bool = False) -> dict:
    ...
    if not is_etf:
        try:
            ...
            if not rev.empty and len(rev) >= 2:
                rev_mom = (rev['revenue'].iloc[-1] / rev['revenue'].iloc[-2] - 1) * 100
                yoy = None
                if len(rev) >= 13:
                    yoy = (rev['revenue'].iloc[-1] / rev['revenue'].iloc[-13] - 1) * 100
                yoy_str = f" 年增{yoy:+.1f}%" if yoy else ''
                rev_signal = f"月增{rev_mom:+.1f}%{yoy_str}"
        except Exception as e:
            print(f'  {sid} 營收取得失敗: {e}')
        try:
            eps_df = get_eps(sid, '2023-01-01')
            if not eps_df.empty:
                e = eps_df['eps'].iloc[-1]
                e1 = eps_df['eps'].iloc[-2] if len(eps_df) >= 2 else e
                eps_qoq = (e - e1) / abs(e1) * 100 if e1 != 0 else 0
                eps_signal = f"EPS {e}（季增{eps_qoq:+.1f}%）"
        except Exception as e:
            print(f'  {sid} EPS取得失敗: {e}')

    # 情緒分析（用已取得的 df 避免重複抓取）
    sentiment = {}
    try:
        df_recent = df[df['date'] >= pd.to_datetime('2025-01-01')]
        if len(df_recent) > 5:
            sentiment = full_sentiment(sid, df_recent)
    except Exception as e:
    ...
    if not is_etf:
        if rev_mom is not None and rev_mom > 5:
            score += 2; reasons.append(f'營收{rev_signal}')
        elif rev_mom is not None and rev_mom > 0:
            score += 1; reasons.append(f'營收{rev_signal}')
    ...
    # 情緒加分
    if sentiment.get('score', 0) >= 2:
        score += 1; reasons.append(f"情緒：{sentiment['verdict']}")
    elif sentiment.get('score', 0) <= -2:
        score -= 1; reasons.append(f"情緒：{sentiment['verdict']}")
    ...
    return {
        ...
        'hist': hist,
        'rev_signal': rev_signal, 'eps_signal': eps_signal,
...
def build_report(mode: str) -> str:
    ...
    for sid, name in STOCK_LIST.items():
        try:
            ...
            print(f'  {name}({sid}) → {r["verdict"]}（score={r["score"]}）')
            if r['verdict'] == 'BUY':
                buy_list.append(r)
    ...
    for sid, name in ETF_LIST.items():
        try:
            ...
            print(f'  {name}({sid}) → ETF score={etf_results[sid]["score"]}')
        except Exception as e:
            print(f'  {name}({sid}) ETF分析失敗: {e}')

    def etf_buy_signal(r, sent) -> tuple:
        """ETF 加碼買入判斷"""
        reasons = []
        score   = 0
    ...
    if buy_list:
        ...
        for r in buy_list:
            ...
            tip = f"{next_trading_day()} 開盤站穩再進，不要追高" if mode == 'close' else "今天開盤確認方向再進"
            ...
            if sent:
                ...
                parts = (sent.get('signals', [])[:1] + sent.get('warnings', [])[:1])
                sent_str = f"\n│ 法人/量能：{sent_icon}{sent['verdict']}（{', '.join(parts)}）" if parts else f"\n│ 法人/量能：{sent_icon}{sent['verdict']}"
    ...
    if watch_list:
        ...
        for r in watch_list:
            missing = "等均線轉多" if r['trend'] != '上升' else "等強陽線確認"
            lines.append(f"  • **{r['name']}（{r['sid']}）** {r['close']}（{r['chg']:+.1f}%）— {missing}")

    # ── 持倉狀況 ──────────────────
    lines.append("")
    lines.append("📦 **ETF 持倉 & 加碼建議**")
    # 從 HOLDINGS 取得不重複的 ETF 持倉
    holding_map = {}
    for h in HOLDINGS:
        if h['id'] not in holding_map:
            holding_map[h['id']] = h

    for sid, info in holding_map.items():
    ...
    lines.append("")
    lines.append(f"_資料：{today}收盤　訊號依據：均線+K線+月營收+EPS+法人情緒_")
    return '\n'.join(lines)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dry', action='store_true', help='只印出不推播')
